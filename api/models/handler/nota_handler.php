...
class NotasHandler
{
    ...
    public function predictNotasPromedioPorMateriaSiguienteSemana()
    {
        // Configurar la localización en español
        setlocale(LC_TIME, 'es_ES.UTF-8');
        ...
        // Consulta para obtener las notas promedio del estudiante por materia en los últimos 3 meses
        $sql = 'SELECT CONCAT(nombre_estudiante, " ", apellido_estudiante) AS estudiante, 
                       nombre AS materia, 
                       AVG(nota) AS promedio_nota,
                       fecha_calificacion
                ...
                WHERE id_estudiante = ? 
                AND fecha_calificacion >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
                GROUP BY estudiante, materia, fecha_calificacion
                ORDER BY materia, fecha_calificacion ASC;';
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($groupedData[$materia])) {
                $groupedData[$materia] = ['dates' => [], 'scores' => []];
            }
            $groupedData[$materia]['dates'][] = $timestamp;
            $groupedData[$materia]['scores'][] = $row['promedio_nota'];
        }
    
        $predictions = [];
        // Calcular la regresión para cada materia
        foreach ($groupedData as $materia => $data) {
            $dates = $data['dates'];
            $scores = $data['scores'];
    
            // Calcular la regresión para predecir las notas promedio en los próximos días de la semana siguiente
            for ($i = 1; $i <= 7; $i++) {
                $X = array_slice($dates, 0, count($dates));
                ...
                // Predecir la nota promedio para el próximo día
                $timestamp = end($dates) + $i * 24 * 60 * 60;
